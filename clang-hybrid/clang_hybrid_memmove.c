...
BEGIN_TEST(clang_hybrid_memmove)
	...
	DEBUG_DUMP_REG(13, 2);
	cmemmove(&t2.pad0[3], &t1.pad0[3], sizeof(t1) - 3);
	check(&t2, 3, 32);
	DEBUG_DUMP_REG(13, 3);
	// ...or an unaligned end...
	cmemmove(&t2, &t1, sizeof(t1) - 3);
	check(&t2, 0, 29);
	DEBUG_DUMP_REG(13, 4);
	// ...or both...
	cmemmove(&t2.pad0[3], &t1.pad0[3], sizeof(t1) - 6);
	check(&t2, 3, 29);
	invalidate(&t2);
	DEBUG_DUMP_REG(13, 5);
	// ...and finally a case where the alignment is different for both?
	cmemmove(&t2, &t1.pad0[1], sizeof(t1) - 1);
	...
	// Simple case: aligned start and end
	DEBUG_DUMP_REG(13, 1);
	cmemmove(&t1, &t2, sizeof(t1));
	check(&t1, 0, 32);
	invalidate(&t1);
	// Test that it still works with an unaligned start...
	DEBUG_DUMP_REG(13, 2);
	cmemmove(&t1.pad0[3], &t2.pad0[3], sizeof(t1) - 3);
	check(&t1, 3, 32);
	DEBUG_DUMP_REG(13, 3);
	// ...or an unaligned end...
	cmemmove(&t1, &t2, sizeof(t1) - 3);
	check(&t1, 0, 29);
	DEBUG_DUMP_REG(13, 4);
	// ...or both...
	cmemmove(&t1.pad0[3], &t2.pad0[3], sizeof(t1) - 6);
	check(&t1, 3, 29);
	invalidate(&t1);
	DEBUG_DUMP_REG(13, 5);
